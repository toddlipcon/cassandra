...
public class Memtable implements MemtableMBean
{
    ...
    class Getter implements Callable<ColumnFamily>
    {
        ...
        {
        	ColumnFamily cf = getLocalCopy(key_, columnFamilyName_);
            if ( cf != null )
                cf = cf.cloneMe();
            return cf;
        }
    ...
    {
    	boolean bVal = isLifetimeViolated();
    ...
    */
    void forceflush(ColumnFamilyStore cfStore, boolean fRecovery) throws IOException
    {
        if(!fRecovery)
        {
	    	RowMutation rm = new RowMutation("Mailbox", flushKey_);
    ...
    */
    ...
    ColumnFamily getLocalCopy(String key, String cfName)
    {
        ...
        else
        {
        	...
        	{
        		column = cFamily.getColumn(values[1] + ":" + values[2]);
        		if(column != null )
        		{
        			columnFamily = new ColumnFamily(cfName_);
            		columnFamily.createColumn(values[1] + ":" + values[2], column.value(), column.timestamp());
        		}
        	}
        }
