...
public final class ColumnFamily implements Serializable
{
    ...
    */
	...
	public void createColumn(String name, byte[] value, int timestamp)
	{
...
class ColumnFamilySerializer implements ICompactSerializer2<ColumnFamily>
{
    ...
    */
    private void fillColumnFamily(ColumnFamily cf, int startPosition, int count, DataInputStream dis) throws IOException
    {
        ...
        else
        {
            if ( startPosition > (size - 1) )
                return;
            count = Math.min(size - startPosition + 1, count);
            if ( count > 0 )
            {
                for ( int i = 0; i < startPosition; ++i )
                {
                    cf.getColumnSerializer().skip(dis);
                }
    ...
    public ColumnFamily deserialize(DataInputStream dis) throws IOException
    {
        ...
        if ( !cf.isMarkedForDelete() )
            fillColumnFamily(cf, -1, 0, dis);
        return cf;
    }

    public ColumnFamily deserialize(DataInputStream dis, int startPosition, int count) throws IOException
    {
        ColumnFamily cf = defreezeColumnFamily(dis);
        if ( !cf.isMarkedForDelete() )
            fillColumnFamily(cf, startPosition, count, dis);
        return cf;
    }
    ...
    */
    public ColumnFamily deserialize(DataInputStream dis, List<String> columnNames) throws IOException
    {
        ...
        {
            int size = dis.readInt();
        	int numEntries = columnNames.size();
        	int count = 0;
        	IColumn column = null;
            for ( int i = 0; i < size; ++i )
            {
            	column = cf.getColumnSerializer().deserialize(dis, columnNames);
            	if(column != null)
            	{
            		cf.addColumn(column.name(), column);
            		++count;
    ...
    public ColumnFamily deserialize(DataInputStream dis, String name, int count) throws IOException
    {
        ...
        if ( !cf.isMarkedForDelete() )
        {
            ...
            for ( int i = 0; i < size; ++i )
            {
	            IColumn column = cf.getColumnSerializer().deserialize(dis, columnName, count);
	            if ( column != null )
	            {
	                cf.addColumn(column.name(), column);
	                break;
